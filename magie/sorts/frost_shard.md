# Éclat de Givre — `frost_shard`

> Projectile : slow court + dégâts soft.
...
*ils font foi dans `WizardCore/src/main/resources/spells.yml`, entrée `frost_shard`.*
...
|---|---|
| École | **Givre** (`FROST`) |
| Incantation | *Glacies fragmentum* |
| Mode | Projectile |
| Couleur | `#7EC8FF` |
...
Projectile de contrôle léger : peu de dégâts, un ralentissement court. Il ouvre l'engagement plutôt qu'il ne le conclut.
...
| Projectile | `PROJECTILE` | `projectile_frostshard.bbmodel` | `animation` | 1.15 s (bouclé) | ORIGIN | TOWARD_TARGET | 1.1 bloc(s) |
...
```text
/magicvfx test frost_shard cast_start
/magicvfx test frost_shard impact
```
