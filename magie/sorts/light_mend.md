# Suture d'Aurore — `light_mend`

> Soin léger sur soi ou un allié proche.
...
*ils font foi dans `WizardCore/src/main/resources/spells.yml`, entrée `light_mend`.*
...
Le soin de base de l'Aurore. L'incantation est volontairement lente : soigner au milieu d'un échange doit être un choix coûteux, pas un réflexe. La colonne de lumière signale clairement aux adversaires que quelqu'un se soigne.
...
| Concentration | `CAST_START` | `light_beam.bbmodel` | `spawn` | 1.3 s | CASTER_FEET | ENTITY_YAW | 2.4 bloc(s) |
...
| Voix d'incantation | `wizardmc.magic.voice.light_1..3` | *(aucun)* |
...
```text
/magicvfx test light_mend cast_start
/magicvfx test light_mend impact
```
